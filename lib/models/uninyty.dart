...
int total() {
  ...
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].iN == 'Penghasilan'
...
int explane() {
  ...
  List a = [0, 0];
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].iN == 'Penghasilan' ? int.parse(history2[i].amount) : 0);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
...
int expenses() {
  ...
  for (var i = 0; i < history2.length; i++) {
    a.add(
      history2[i].iN == 'Penghasilan' ? 0 : int.parse(history2[i].amount) * -1,
    );
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
